...
/*
 * A REAL GIT REPOSITORY, because writeSnapshot now asks git whether the tree it
 * is about to adopt as normal is clean. A plain temp directory is unmeasurable,
 * and unmeasurable is refused -- so a bare mkdtemp fixture would exercise the
 * refusal path on every test instead of the behaviour each one is about.
 *
 * It is also more faithful: production always runs inside a repository, and the
 * old fixture tested writeSnapshot somewhere it can never actually run.
 */
