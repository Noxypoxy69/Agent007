...
test('precondition: the shipped lists loaded and contain the variables asserted by behaviour below', () => {
  ...
  assert.ok(AMBIENT_ONLY_VARS.includes('GIT_INDEX_FILE'), 'GIT_INDEX_FILE left the ambient-only list');
});
...
for (const spelling of casings('GIT_INDEX_FILE')) {
  test(`${spelling}: an AMBIENT index is dropped`, () => {
    assert.equal(withAmbient(spelling, BOGUS_INDEX, () => indexPath()), norm(path.join(GENUINE.gitDir, 'index')),
      `an inherited ${spelling} chose the index git used`);
  });
