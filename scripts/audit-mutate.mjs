...
const MUTATIONS = [
  ...
  ['M7 revokeDecision (src): revert to the compare-against-itself form', 'src/ownerDecisions.mjs',
    `  if (!isOwnerId(by, owners)) {`,
    `  if (isNonEmptyString(d.owner_id) && by !== d.owner_id) {`],
  ['M7b revokeDecision (src): remove the author check entirely', 'src/ownerDecisions.mjs',
    `  if (!isOwnerId(by, owners)) {`,
    `  if (false) {`],
  ['M15 CLI CONTRACT: append a suffix to the cursor line (still 2 spaces)', 'bin/agentbridge.mjs',
    "        console.log(`  cursor  ${cursor}`);",
    "        console.log(`  cursor  ${cursor}  (server)`);"],
  ['M16 CLI CONTRACT: drop the leading indent of the cursor line', 'bin/agentbridge.mjs',
    "        console.log(`  cursor  ${cursor}`);",
    "        console.log(`cursor  ${cursor}`);"],
  ['M17 advanceCursor: drop the forward-only guard (allow rewinds)', 'scripts/bridge-session-poll.mjs',
    `    if (!Number.isFinite(t) || t <= bestMs) continue;`,
    `    if (!Number.isFinite(t)) continue;`],
  ['M18 advanceCursor: drop the line anchors (rule 13 regression)', 'scripts/bridge-session-poll.mjs',
    `    const m = /^\\s*cursor\\s+(\\S+)\\s*$/.exec(line);`,
    `    const m = /cursor\\s+(\\S+)/.exec(line);`],
  ['M19 supervisor: treat a null status as a hard error again', 'scripts/bridge-session-poll.mjs',
    `    if (r.status === null) continue;`,
    `    if (r.status === null) { await new Promise((z) => { setTimeout(z, 1); }); continue; }`],
